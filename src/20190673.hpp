...
string binString(const Eigen::Vector<int,12> & data){
    string dab = "000000000000";
    int idx = 0;
    for(auto row:data.rowwise()){
        for(auto e:row){
            if(e == 1){dab[idx] = '1';}
...
string binString(const string & dab){
    string binS = "000000000000";
    int idx = 0;
    for(auto c:dab){
        if(c == '-'){binS[idx] = '1';}
        idx += 1;
...
public:
    ...
    void parseTrans(int idx,bool verbo = true){
        ...
        for(auto a:oActions){
            ...
            //(TRANS) (TRICKY!) add proper probability to each opponent action
            double de = 1; //denominator for probability
            for(int t = 0;t<size(a);t++){de = de*(empty-t);} // multiple actions need to be accounted by the law of multiplication
    ...
    void parse(bool verbo = true){
        ...
        if(verbo){outStream << "[PARSE] Parsing State ["<<dataString<<"]" << endl;}

        parseActions();
        for(int idx = 0;idx < actions.size();idx++){parseTrans(idx,verbo);}

        policy = int(rand()) % size(actions); //initialize with a random policy
        if(verbo){outStream << " ++ Policy Initialized To [" << actions[policy] << "](idx: "<<policy <<")"<<endl;}
...
ostream & operator<< (ostream& os, State& s) {
    ...
    os << '[' << binString(s.dataString) << ']';
    os << " <" << s.numClosed << "/4>";
    os << " {" << (s.terminal ? 'T':'-') << "}";
    os << " (V:" << s.v << ")";
    if(!s.terminal && s.parsed){os << " policy: " << s.actions[s.policy];}
...
State* policyIteration(const Eigen::Vector<int, 12>& state,const string& logPath = "./log",bool verbo = false){

    // timing utility setup
    const string& ext = ".txt";

    auto start = chrono::high_resolution_clock::now();
    auto now = start;

    // logfile utility setup
    ofstream ofs(logPath + ext);
    ostream& os = ofs;
    //ostream& os = cout;
    ...
    // create / connect all relevant states
    auto s = stateLoop(state,&stateMap,os,verbo);
    ...
    os << "[PI] Finished!" << endl;
    if(verbo) {
        os << "[PI-FIN] Outputting Statemap to: " << logPath + "-MAP" + ext << endl;

        ofstream ofsMap(logPath + "-MAP" + ext);
        ostream &osMap = ofsMap;
        osMap << stateMap << endl;


        os << "[PI-FIN] Outputting Final Analysis to:" << logPath + "-ANAL" + ext << endl;

        ofstream ofsAnal(logPath + "-ANAL" + ext);
        ostream &osAnal = ofsAnal;
        s->verbose(osAnal);
    }

    os << "[PI] Finished (no further policy updates) in " << iter << " iterations" << endl;
...
double getOptimalValue(const Eigen::Vector<int, 12>& state){
    ...
    auto path = "./log_[" + binString(state) + "]-V";
    return policyIteration(state,path, false) -> v; //TOGGLE VERBOSITY HERE
}
