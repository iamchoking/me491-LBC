...
def session(unit_updates=750, viz_duration=30, given_target = None,given_opponent=None,given_config=None):
    ...
    if target_type == opponent_type:
        # if self-train, choose randomly from previous 4 (don't choose too close to current)
        index = random.choice([2, 2, 3, 3, 3, 4])
        # print(possible_opponents)
        opponent_name = possible_opponents[index]
        session_logger.info("[SESSION] Chose Self Play Opponent: "+opponent_name)
    else:
...
if __name__ == "__main__":
    prescribed_routine = [
        ...
        ['STABLE32','AGILE','ffa-fine.yaml',750],
        ['STABLE32','PASSIVE','standard-fine.yaml',750],
        # self-training makes things worse!
        # ['STABLE32','STABLE32','ffa-fine.yaml',450],
        # ['STABLE32','STABLE32','ffa-fine.yaml',451],
    ]
    ...
    while True:
        ...
        for [target,opponent,config,updates] in prescribed_routine:
            if first_pass and (target != prescribed_routine[starting_index][0] or opponent != prescribed_routine[starting_index][1] or updates != prescribed_routine[starting_index][3]):
                session_logger.info("[SESSION] Skipping to starting index, ( skipped " + target + " vs " + opponent + ")")
                continue
            else:
                first_pass = False
