...
class Game:
    def __init__(self,
        ...
        copy_files([self.opp_path, opp_mean_path, opp_var_path, self.task_dir + "/Environment.hpp",
                    self.task_dir + "/AnymalController_20190673.hpp"], self.game_dir + "/oppoment_" + self.timestr)
    ...
    def ppo_outer_loop(self, updates=5000, do_viz=True):
        ...
        for update in range(updates): # TODO implement (keyboard interrupt)
            ...
            elif metrics['score'] > 50 and self.update_num < 100:
                self.logger.info("[GAME-OUTER][TRIVIAL] Game Session finished (score exceeds 50)")
                break
            if update == updates - 1:
