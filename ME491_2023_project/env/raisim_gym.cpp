...
PYBIND11_MODULE(RAISIMGYM_TORCH_ENV_NAME, m) {
  py::class_<VectorizedEnvironment<ENVIRONMENT>>(m, RSG_MAKE_STR(ENVIRONMENT_NAME))
    ...
    .def("getObDims", &VectorizedEnvironment<ENVIRONMENT>::getObDims)
    .def("getActionDims", &VectorizedEnvironment<ENVIRONMENT>::getActionDims)
    ...
    .def("getMetrics", &VectorizedEnvironment<ENVIRONMENT>::getMetrics)
